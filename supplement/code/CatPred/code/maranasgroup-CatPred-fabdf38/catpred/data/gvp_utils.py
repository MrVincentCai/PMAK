...
class ProteinGraphDataset(data.Dataset):
    ...
    def __init__(self, data_list, 
                 ...
                 top_k=30, num_rbf=16, include_esm_feats = True, device="cpu"):
    ...
    def _featurize_as_graph(self, protein):
        ...
        with torch.no_grad():
            ...
            seq_feats = torch.as_tensor(protein['esm2_feats'], device=self.device)
            if self.include_esm_feats and (seq.shape[0]!=seq_feats.shape[0]): 
                seq = seq[:seq_feats.shape[0]]
                coords = coords[:seq_feats.shape[0],:,:]
            ...
            if self.include_esm_feats:
                node_s = torch.cat([dihedrals, seq_feats], dim=-1)
            else:
                node_s = dihedrals
...
class ProteinGraphDataset_EnTransformer(data.Dataset):
    '''
    A map-syle `torch.utils.data.Dataset` which transforms JSON/dictionary-style
    protein structures into featurized protein graphs as described in the 
    manuscript.
    ...
    :param device: if "cuda", will do preprocessing on the GPU
    '''
    def __init__(self, data_list, include_esm_feats = True, device="cpu"):
